...
for _R_1x, _R_1N, _dR_1x, _dR_1N in zip(list_R_1x, list_R_1N, list_dR_1x, list_dR_1N):
    R_1x = Values(name=r"R_{1x}", values=_R_1x, delta=_dR_1x, unit=r"\text{Ом}", roundoff=1)
    R_1N = Values(name=r"R_{1N}", values=_R_1N, delta=_dR_1N, unit=r"\text{Ом}", roundoff=1)
    Epsilon_N = Values(name=r"\mathcal{E}_N", values=1.016, delta=0.001, roundoff=3)

    form = Formula(R_1x / R_1N * Epsilon_N, data=[R_1N, R_1x, Epsilon_N], name=r"\mathcal{E}_x", roundoff=3)

    print(form.indetect_error.latex())